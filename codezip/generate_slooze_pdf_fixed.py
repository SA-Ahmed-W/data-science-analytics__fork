...
class SloozePDF(FPDF):
    ...
    def bullet_list(self, items, bold_prefix=True):
        ...
        for item in items:
            ...
            if bold_prefix and ':' in item:
                ...
                bullet_width = 5
                indent = 5
                available_width = 190 - indent - bullet_width
                
                # Start new line with indent
                self.cell(indent)
                self.cell(bullet_width, 6, chr(149), align='L')  # Bullet
                
                # Bold prefix
                self.set_font('helvetica', 'B', 10)
                prefix_text = parts[0] + ':'
                prefix_width = self.get_string_width(prefix_text + ' ')
                self.cell(prefix_width, 6, prefix_text + ' ', align='L')
                
                # Regular content
                self.set_font('helvetica', '', 10)
                self.multi_cell(available_width - prefix_width, 6, parts[1].strip())
            else:
                self.cell(5)  # Indent
                self.cell(5, 6, chr(149), align='L')  # Bullet
    ...
    def key_finding_box(self, title, content):
        ...
        self.line(15, start_y, 15, start_y + 30)
        
        # Add background (light gray)
        self.set_fill_color(250, 250, 250)
        self.rect(17, start_y, 178, 30, 'F')
...
def create_slooze_documentation():
    ...
    pdf.set_xy(65, 63)
    pdf.cell(80, 6, 'DATA SCIENCE CHALLENGE', align='C')
    
    # Main title
    pdf.set_xy(15, 100)
    pdf.set_font('helvetica', 'B', 28)
    pdf.set_text_color(255, 255, 255)
    pdf.cell(180, 15, 'Inventory Optimization &', new_x="LMARGIN", new_y="NEXT", align='C')
    pdf.cell(180, 15, 'Supply Chain Analysis', new_x="LMARGIN", new_y="NEXT", align='C')
    
    # Subtitle
    pdf.set_xy(15, 145)
    pdf.set_font('helvetica', '', 14)
    pdf.set_text_color(200, 200, 200)
    pdf.cell(180, 10, 'Slooze Take-Home Challenge', new_x="LMARGIN", new_y="NEXT", align='C')
    ...
    pdf.cell(180, 8, 'Technical Documentation', new_x="LMARGIN", new_y="NEXT", align='C')
    pdf.cell(180, 8, 'Comprehensive Analysis Report', new_x="LMARGIN", new_y="NEXT", align='C')
    pdf.cell(180, 8, datetime.now().strftime('%B %Y'), new_x="LMARGIN", new_y="NEXT", align='C')
    ...
    for item, page in toc_items:
        ...
        pdf.cell(150, 7, item.strip(), align='L')
    ...
    pdf.cell(0, 10, '--- End of Documentation ---', align='C')
